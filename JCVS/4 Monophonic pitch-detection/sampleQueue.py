...
class SampleQueue(object):
	def __init__(self, fftResolution, bufferSize, dtype):
		...
		self.__fftRes__ = max(fftResolution, bufferSize)
		self.__bufSize__ = bufferSize
		self.__sampleDtype__ = dtype

		self.__ifftStartIndex__ = self.__fftRes__ - bufferSize

		self.__bufCount__ = int(np.ceil(self.__fftRes__ / bufferSize))
